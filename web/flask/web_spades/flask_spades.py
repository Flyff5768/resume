...
from Spades_Team.database.db_mysql import connect_mysql,mysql_select
...
def keyword2():
    ...
    return render_template('keyword.html',place_list=elasticsearch_search(keyword),catogory = keyword)


@app.route('/Parent_child',methods=['GET','POST'])
def Parent_child():
    return render_template('catogory.html',place_list=select_sql('Parent_child_score','score'),catogory = '親子')

@app.route('/Couple',methods=['GET','POST'])
def Couple():
    return render_template('catogory.html',place_list=select_sql('Couple_score','score'),catogory = '情侶')

@app.route('/Old_and_young',methods=['GET','POST'])
def Old_and_young():
    return render_template('catogory.html',place_list=select_sql('Old_and_young_score','score'),catogory = '老少咸宜')

@app.route('/friend',methods=['GET','POST'])
def friend():
    return render_template('catogory.html',place_list=select_sql('friend_score','score'),catogory = '朋友')
...
def keelung_hot():
    return render_template('hot_pot.html',place_list=select_sql('keelung_hot','"count(attractions)"'),catogory = '基隆')

@app.route('/taipei_hot',methods=['GET','POST'])
def taipei_hot():
    return render_template('hot_pot.html',place_list=select_sql('taipei_hot','"count(attractions)"'),catogory = '台北')

@app.route('/new_taipei_hot',methods=['GET','POST'])
def new_taipei_hot():
    return render_template('hot_pot.html',place_list=select_sql('new_taipei_hot','"count(attractions)"'),catogory = '新北')

@app.route('/taoyuan_hot',methods=['GET','POST'])
def taoyuan_hot():
    return render_template('hot_pot.html',place_list=select_sql('taoyuan_hot','"count(attractions)"'),catogory = '桃園')

@app.route('/hsinchu_hot',methods=['GET','POST'])
def hsinchu_hot():
    return render_template('hot_pot.html',place_list=select_sql('hsinchu_hot','"count(attractions)"'),catogory = '新竹')

@app.route('/yilan_hot',methods=['GET','POST'])
def yilan_hot():
    return render_template('hot_pot.html',place_list=select_sql('yilan_hot','"count(attractions)"'),catogory = '宜蘭')
