    ...
    try:
        ...
        for item in items:
            if item.text().strip().startswith('window.__additionalDataLoaded'):
                ...
                json_place_key_list = ['文章網址','發文時間','標題','景點名稱','文章內容','留言','地址','縣市']
                json_place_value_list = ['each_article_url', 'posting_time', 'title', 'title', 'content', '留言', 'NA', '縣市']

                #同整存檔內容
                global each_article_url
                total = '{ "":"' +  + '","":"'+  +'","":"' +  +'","":"NA"' +',"":"' +  + '","":"' + comment.replace('\n','') +'","":""'+',"":"台北市"' +'}'
                print(total)
...
if __name__ == '__main__':
    ...
    tag_list = ['台北','新北','基隆', '桃園', '新竹','宜蘭']  # 手動更改
    tag_list = ['景點', '美食']  # 手動更改
